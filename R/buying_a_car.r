...
# Furthermore this percent of loss increases by 0.5 percent at the end of every two months. Our man finds it difficult to make all these calculations.
#
# Can you help him?
#
...
nbMonths <- function(priceOld, priceNew, saving, loss) {
    oldCarPrice = priceOld
    newCarPrice = priceNew
    savings = 0
    months = 0
    ...
    while (oldCarPrice + savings < newCarPrice) {
      ...
      oldCarPrice = oldCarPrice * (1 - (loss/100))
      newCarPrice = newCarPrice * (1 - (loss/100))
    }
    
    moneyLeft = savings + oldCarPrice - newCarPrice
    
    return(c(months, round(moneyLeft)))
}