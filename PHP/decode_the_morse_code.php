...
···−−−···. These special codes are treated as single special characters, and usually are transmitted as
separate words.
...
function decode_morse(string $code): string {
  ...
  for ($i = 0; $i < count($words); $i++) {
    ...
    foreach ($letters as $letter)
      $humanLetters .= MORSE_CODE[$letter];
    
    if ($humanLetters !== "")
      array_push($humanWords, $humanLetters);
  }
  
  $decoded = "";
  for ($i = 0; $i < count($humanWords); $i++) {
    $decoded .= $humanWords[$i];
    if ($i < count($humanWords)-1) {
      if ($decoded != "")
        $decoded .= " ";
    }
  }
